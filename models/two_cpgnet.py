...
class AttNet(nn.Module):
    ...
    def build_network(self):
        ...
        self.point_pre_2 = backbone.PointNetStacker(self.point_feat_out_channels, rv_context_layer[0], pre_bn=True,
                                                    stack_num=2)
        self.bev_net_2 = bird_view.BEVNet(bev_base_block, bev_context_layer, bev_layers, use_att=True)
        self.rv_net_2 = range_view.RVNet(rv_base_block, rv_context_layer, rv_layers, use_att=True)
        self.point_post_2 = eval('backbone.{}'.format(fusion_mode))(in_channel_list=point_fusion_channels,
                                                                    out_channel=self.point_feat_out_channels,
                                                                    way=fusion_way)
    ...
    def stage_forward(self, point_feat, pcds_coord, pcds_sphere_coord):
        ...
        bev_input = VoxelMaxPool(pcds_feat=point_feat, pcds_ind=pcds_coord[:, :, :2].contiguous(),
        ...
        # range-view
        rv_input = VoxelMaxPool(pcds_feat=point_feat, pcds_ind=pcds_sphere_coord_cur, output_size=self.rv_shape,
                                scale_rate=(1.0, 1.0))
        rv_feat = self.rv_net(rv_input)
        point_rv_feat = self.rv_grid2point(rv_feat, pcds_sphere_coord_cur)
        # merge multi-view
        point_feat_out = self.point_post(point_feat, point_bev_feat, point_rv_feat)

        point_feat = self.point_pre_2(point_feat_out)

        bev_input = VoxelMaxPool(pcds_feat=point_feat, pcds_ind=pcds_coord[:, :, :2].contiguous(),
                                 output_size=self.bev_wl_shape, scale_rate=(1.0, 1.0))  # (BS, C, H, W)
        bev_feat = self.bev_net_2(bev_input)
        point_bev_feat = self.bev_grid2point(bev_feat, pcds_cood_cur)

        rv_input = VoxelMaxPool(pcds_feat=point_feat, pcds_ind=pcds_sphere_coord_cur, output_size=self.rv_shape,
                                scale_rate=(1.0, 1.0))
        rv_feat = self.rv_net_2(rv_input)
        point_rv_feat = self.rv_grid2point(rv_feat, pcds_sphere_coord_cur)
        point_feat_out = self.point_post_2(point_feat, point_bev_feat, point_rv_feat)
