...
class DataloadVal(Dataset):
    ...
    def __getitem__(self, index):
        ...
        pad_length_list = [0 for i in range(len(pc_list))]
        # resample
        for ht in range(1, len(pc_list)):
            choice = np.random.choice(pc_list[ht].shape[0], pc_list[0].shape[0], replace=True)
            pc_list[ht] = pc_list[ht][choice]
            pc_label_list[ht] = pc_label_list[ht][choice]
...
if __name__ == "__main__":
    ...
    train_loader = DataLoader(train_dataset,
                            ...
                            num_workers=0,
                            pin_memory=True)
    for i, (pcds_xyzi, pcds_coord, pcds_sphere_coord, pcds_target, pcds_xyzi_raw, pcds_coord_raw, pcds_sphere_coord_raw, meta_list) in tqdm.tqdm(enumerate(train_loader)):
        print(i, pcds_coord.shape, pcds_sphere_coord.shape)

        print(pcds_coord[:, 0, :, :2].contiguous().shape, pcds_sphere_coord[:, 0].contiguous().shape)
        exit()